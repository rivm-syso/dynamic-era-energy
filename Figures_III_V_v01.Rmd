...
for(s in 1:length(emis.scens[,1])){
  Emis.loc_NR <- paste0("data/Emis PV data III-V/PV_emissions_v1.8_",
                        emis.scens$Scales[s],"_NR_",
                        emis.scens$Substances[s],"_kg.xlsm") 
  Emis.loc_R <- paste0("data/Emis PV data III-V/PV_emissions_v1.8_",
...
p <- ggplot(subset(sum.stat.long2, sum_stat %in% "geo.mean"), aes(x=year, y=PEC)) +
  ylab('Concentration (ug/L or mg/kg dw)') +
  xlab('Time (years)')+
  geom_ribbon(aes(ymin = subset(sum.stat.long2, sum_stat %in% "P25")$PEC, 
                  ymax = subset(sum.stat.long2, sum_stat %in% "P75")$PEC), 
              fill = brewer.pal(7,"Greens")[2])+
  geom_line(position = "identity", alpha = 1, colour=brewer.pal(7,"Greens")[7])

p1 <- p + facet_grid(vars(compartment),vars(scenario), scales = "free")+ theme_bw()
...
rq_w$PNEC[rq_w$chemical=="As"] = 5.6 #JG-MKN zoet oppervlaktewater in microgram/L
rq_w$PNEC[rq_w$chemical=="Ga"] = 103 #JG-MKN zoet oppervlaktewater in microgram/L
rq_w$PNEC[rq_w$chemical=="In"] = 40.6 #JG-MKN zoet oppervlaktewater in microgram/L
...
rq_s$PNEC[rq_s$chemical=="As"] = 2.9 #3 PNEC terrestrial organisms ECHA registration dossier
rq_s$PNEC[rq_s$chemical=="Ga"] = 2.7 #PNEC terrestrial organisms ECHA registration dossier
rq_s$PNEC[rq_s$chemical=="In"] = 7.3 #PNEC terrestrial organisms ECHA registration dossier
...
p +
  facetted_pos_scales(
    y = list(
      chemical == "As" ~ scale_y_log10(limits = c(1e-11,1)
                                       ),
      chemical == "Ga" ~ scale_y_log10(limits = c(1e-15,1e-4)
                                       ),
      chemical == "In" ~ scale_y_log10(limits = c(1e-16,1e-7)
                                       ) 
    )
  )
...
Below first the sensitivity analysis is conducted based on the input and output data. Followed by making a figure to show the results.
...
borg_d <- data.frame()
substances <- c("As","Ga","In")   ### define substances for which to make the analysis
option <- c("NR","R") ### define scenarios for which to make the analysis
for (s in substances){
  for (o in option){
    for (c in comp){
      if (c %in% c("wL","sL")){
        X_emi <- head(read_excel(paste0("data/Emis PV data III-V/PV_emissions_v1.8_LOC_",o,"_",s,"_kg.xlsm"),sheet = "PROB_X")[,-1], 1000)
        }else if (c %in% c("w1R","s2R")){
          X_emi <- head(read_excel(paste0("data/Emis PV data III-V/PV_emissions_v1.8_AM_",o,"_",s,"_kg.xlsm"),sheet = "PROB_X")[,-1], 1000)
        }else{
          X_emi <- head(read_excel(paste0("data/Emis PV data III-V/PV_emissions_v1.8_EU_",o,"_",s,"_kg.xlsm"),sheet = "PROB_X")[,-1], 1000)
          }
      #store rows with errors to remove from X and Y
      bad_rows <- unique(which(is.na(mat.yr),arr.ind = TRUE)[,2])
      X_sb <- out.data2[[paste0(s,"_",o,"_III-V")]][["addriskinput"]][-c(1:3),]
      rownames(X_sb) <- NULL # Resetting index numbers of rows
      probX <- cbind(X_emi, X_sb)
      #Build Y vector from output
      probY <- data.frame(RQ = rq_t$RQ[rq_t$scenario == paste0(s,"_",o,"_III-V") & rq_t$compartment == c])
      probY[probY==0] <- 1e-20
      #probY <- probY[-bad_rows,]        
      ...
      probX2 <- subset(probX1, select = !(colnames(probX1) %in% c("Kpsoil.silver","Kpsoil.As","Kpsoil.copper","Kpsoil.Ga","Kpsoil.In","Kpsoil.lead","to.sT.(s-1)./.from.sT","v./.aL","v./.wL","v./.sdL","v./.aR","v./.w1R","v./.sd1R","v./.aC","v./.w1C","v./.sd1C","v./.sd2C","v./.w0C", "v./.w2C", "v./.s1C", "v./.s2C", "v./.s3C", "v./.aM", "v./.w2M","v./.w3M","v./.sdM", "v./.sM")))
      #run global sensitivity analysis
      m <- sensiFdiv(model = NULL, X=probX2, fdiv = "TV", nboot = 0, conf = 0.95,   scale = TRUE)
      tell(m, y=probY,S)
      ggplot(m, ylim = c(0, 1))
      #prepare output for ggplot
      borg_d_temp <- data.frame(colnames(probX2),m$S$original)
      names(borg_d_temp)<-c("Factor", "delta")
      borg_d_temp$Scenario<-c(paste0(s,"_",o,"_III-V"))
      borg_d_temp$Compartment<-c
      borg_d <- bind_rows(borg_d_temp,borg_d)
    }
  }
}
save(borg_d, file=paste0("data/borg_d.RData"))

```

```{r figure 7 sensitivity}

load("data/borg_d.RData")
  
#heatmap

borg_d$Factor <- factor(borg_d$Factor, levels = unique(borg_d$Factor))

unique(borg_d$Factor)

p <- ggplot(borg_d, aes(x=Compartment, y=Factor, fill= delta)) + 
...
p + geom_tile(borg_d, mapping = aes(x=Compartment, y=Factor), fill="transparent", colour="white", size=1)+ facet_grid(vars(),vars(Scenario), scales = "free")

#  To get a better formatted figure it is required to export to excel and change variable order and names
#  !!! introduce in the new sheet named "newlvl" the preferred order and names for the sensitivity factors

# wb <- createWorkbook()
# addWorksheet(wb, "oldlvl")
# old.lvl<-levels(borg_d$Factor)
# writeData(wb, sheet = 1, data.frame(old.lvl))
# addWorksheet(wb, "newlvl")
# new.lvl <- data.frame(matrix(ncol = 2, nrow = 0))
# colnames(new.lvl) <- c("Variable","Name")
# writeData(wb, sheet= 2, new.lvl)
# saveWorkbook(wb, "Names_borg_d.xlsx")

# Import new order and names of variables
new.lvl <- read.xlsx("Names_borg_d.xlsx",sheet="newlvl")

borg_d$Factor <- factor(borg_d$Factor, levels = rev(new.lvl$Variable))
...
p + geom_tile(borg_d, mapping = aes(x=Compartment, y=Factor), fill="transparent", colour="white", size=1)+ facet_grid(vars(),vars(Scenario), scales = "free") + scale_y_discrete(labels=rev(new.lvl$Name))

ggsave(filename = paste0("GSA_v01.jpg"),
       plot = last_plot(),
       device = "jpeg",
       path = "figures",
       scale = 2,
