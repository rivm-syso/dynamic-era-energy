...
p1 <- p + facet_grid(vars(scenario), vars(emission_rout), scales = "free") +scale_y_continuous(trans='log10')+ theme_bw()
...
p1 <- p + facet_grid(vars(scenario), vars(use_eol), scales = "free")+ theme_bw()
...
p1 <- p + facet_grid(vars(scenario), vars(use_eol), scales = "free")+scale_y_continuous(trans='log10')+ theme_bw()
...
ggsave(filename = paste0("emission_LOCAL_useeol_Pb_LOG_",cases$paneltype[1],"_v03.jpg"),
       plot = p1,
       device = "jpeg",
       path = "figures",
       scale = 1.5,
...
LOC_Pb_NR <- LOC_As2[LOC_As2$substance=="Pb"&LOC_As2$use_eol =="EOL_NR",]
p <- ggplot(subset(LOC_Pb_NR, sum_stat %in% "geo.mean"), aes(x=year, y=emmision)) +
  ylab('Emission (kg)') +
  xlab('Time (years)')+
  geom_ribbon(aes(ymin = subset(LOC_Pb_NR, sum_stat %in% "P25")$emmision, 
                  ymax = subset(LOC_Pb_NR, sum_stat %in% "P75")$emmision),
              fill = brewer.pal(7,"Blues")[2])+
  geom_line(position = "identity", alpha = 1, colour=brewer.pal(7,"Blues")[7])

p1 <- p +  theme_bw() #+scale_y_continuous(trans='log10')

ggsave(filename = paste0("emission_LOCAL_useeol_Pb_NR_",cases$paneltype[1],"_v01.jpg"),
       plot = p1,
       device = "jpeg",
       path = "figures",
       scale = 1,
...
#Factsheet figure 3. 
ggsave(filename = paste0("emission_LOCAL_useeol_Pb_NR_",cases$paneltype[1],"_v01.svg"),
       plot = p1,
       device = "svg",
       path = "figures",
       scale = 1,
...
p <- ggplot(subset(sum.stat.long2, sum_stat %in% "geo.mean"), aes(x=year, y=PEC)) +
  ...
  xlab('Time (years)')+
  geom_ribbon(aes(ymin = subset(sum.stat.long2, sum_stat %in% "P5")$PEC, 
                  ymax = subset(sum.stat.long2, sum_stat %in% "P95")$PEC),
              fill = brewer.pal(7,"Greens")[2])+
  geom_line(position = "identity", alpha = 1, colour=brewer.pal(7,"Greens")[7])


p1 <- p + facet_grid(vars(scenario),vars(compartment), scales = "free")+ theme_bw()

ggsave(filename = paste0("PEC_",cases$paneltype[1],"_v03.jpg"),
       plot = last_plot(),
       device = "jpeg",
       path = "figures",
       scale = 2,
...
p1 <- p + facet_grid(vars(scenario),vars(compartment), scales = "free")+ theme_bw() +scale_y_continuous(trans='log10')+ylab('LOG-Concentration (ug/L or mg/kg dw)') 

ggsave(filename = paste0("PEC_LOG_",cases$paneltype[1],"_v03.jpg"),
       plot = last_plot(),
       device = "jpeg",
       path = "figures",
       scale = 2,
...
p2 <- p1 + facet_grid(vars(scenario),vars(compartment), scales = "free")+ theme_bw()

ggsave(filename = paste0("PEC_Ag_Cu_Pb_RLIM_v03.jpg"),
       plot = last_plot(),
       device = "jpeg",
       path = "figures",
       scale = 2,
...
#Plots for both R and NR scenario
sum.stat.Ag.Cu.Pb_R <- sum.stat.long2

risklimits.Ag.Cu.Pb_R <- risklimits

unique(sum.stat.long2$scenario)
sum.stat.Pb <- sum.stat.long2[(sum.stat.long2$scenario=="Pb_NR_Tandem" | 
                                  sum.stat.long2$scenario=="Pb_R_Tandem")&
                                (sum.stat.long2$compartment=="sL"|sum.stat.long2$compartment=="wL"),]

pR <- ggplot(subset(sum.stat.Ag.Cu.Pb_R, sum_stat %in% "geo.mean"), aes(x=year, y=PEC)) +
  ylab('Concentration (ug/L or mg/kg dw)') +
  xlab('Time (years)')+
  geom_ribbon(aes(ymin = subset(sum.stat.Ag.Cu.Pb_R, sum_stat %in% "P5")$PEC, 
                  ymax = subset(sum.stat.Ag.Cu.Pb_R, sum_stat %in% "P95")$PEC),
              fill = brewer.pal(7,"Greens")[2])+
  geom_line(position = "identity", alpha = 1, colour=brewer.pal(7,"Greens")[7])

p1R <- pR + geom_hline(data = risklimits.Ag.Cu.Pb_R, aes(yintercept = rlim))

p2R <- p1R + facet_grid(vars(scenario),vars(compartment), scales = "free")+ theme_bw()

ggsave(filename = paste0("PEC_Ag_Cu_Pb_RLIM_R_v03.jpg"),
       plot = last_plot(),
       device = "jpeg",
       path = "figures",
       scale = 2,
...
p3R <- p1R + facet_grid(vars(scenario),vars(compartment), scales = "free")+ theme_bw() +scale_y_continuous(trans='log10')+ylab('LOG-Concentration (ug/L or mg/kg dw)') 
...
unique(sum.stat.Pb$scenario)
unique(sum.stat.Pb$compartment)

p <- ggplot(subset(sum.stat.Pb, sum_stat %in% "geo.mean"), aes(x=year, y=PEC)) +
  ylab('Concentration (ug/L or mg/kg dw)') +
  xlab('Time (years)')+
  geom_ribbon(aes(ymin = subset(sum.stat.Pb, sum_stat %in% "P5")$PEC, 
                  ymax = subset(sum.stat.Pb, sum_stat %in% "P95")$PEC),
              fill = brewer.pal(7,"Greens")[2])+
  geom_line(position = "identity", alpha = 1, colour=brewer.pal(7,"Greens")[7])
...
ggsave(filename = paste0("PEC_Pb_",cases$paneltype[1],"_v03.jpg"),
       plot = last_plot(),
       device = "jpeg",
       path = "figures",
       scale = 2,
...
sum.stat.Pb.NR <- sum.stat.long2[(sum.stat.long2$scenario=="Pb_NR_Tandem"   )&
                                (sum.stat.long2$compartment=="wL"),]


p <- ggplot(subset(sum.stat.Pb.NR, sum_stat %in% "geo.mean"), aes(x=year, y=PEC)) +
  ylab('Concentration (ug/L)') +
  xlab('Time (years)')+
  geom_ribbon(aes(ymin = subset(sum.stat.Pb.NR, sum_stat %in% "P25")$PEC, 
                  ymax = subset(sum.stat.Pb.NR, sum_stat %in% "P75")$PEC),
              fill = brewer.pal(7,"Greens")[2])+
  geom_line(position = "identity", alpha = 1, colour=brewer.pal(7,"Greens")[7])
...
ggsave(filename = paste0("PEC_Pb_NR",cases$paneltype[1],"_v01.jpg"),
       plot = p1,
       device = "jpeg",
       path = "figures",
       scale = 1,
...
ggsave(filename = paste0("PEC_Pb_NR",cases$paneltype[1],"_v01.pdf"),
       plot = p1,
       device = "pdf",
       path = "figures",
       scale = 1,
...
ggsave(filename = paste0("PEC_Pb_NR",cases$paneltype[1],"_v01.svg"),
       plot = p1,
       device = "svg",
       path = "figures",
       scale = 1,
...
# aim to calculate Cummulative stock instead of PEC/RQ for sensitivity analysis

#Build Y vector from output
probY <- data.frame(RQ = rq_t$RQ[rq_t$scenario == "Ag_NR_Tandem" & rq_t$compartment == "wL"])


probY[probY<=0] <- 1e-20 ##negative risks?
#probY <- probY[-bad_rows,] #Commented as bad rows are already removed in rq df bt which probY is defined
...
borg_d$Scenario<-c("Ag_NR_Tandem")
